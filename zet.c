...
"file.\n\n"
"\n");
			printf("%s -f s [options]\n", argv[0]);
			printf("\t-f s\tpartitionname\n"
			 "\t-l i\tsize in count of blocks\n"
			 "\t-b i\tblocksize given in count of 512 byte\n"
			 "\t-s\tsync mode, syncs each block\n"
			 "\t-n\tskip initilizing\n"
			 "\t-q\tjust do a sequencial check\n");
			exit(-1);
		}
	if (n==NULL)
	{
	...
	if ((f = open(n, O_RDWR|(syncmode?O_SYNC:0))) < 0)
	{
		printf("%s error %d\n", argv[0], errno);
		exit(-1);
	}
	...
	if (initmode)
	{
		...
		do
		{
			printf("write at %d       \r", tell(f));
			fflush(stdout);
			if (fsize && tell(f) > fsize * blocksize)
				break;
			fill(buffer, blocksize);
		} while (write(f, buffer, blocksize) == blocksize);
		fsize = tell(f) / blocksize;
	}
	if (sequmode)
	{
		lseek(f, 0, SEEK_SET);
		while (read(f, buffer, blocksize) == blocksize)
			...
			{
				printf("read at %d       \r", tell(f));
				fflush(stdout);
				if (fsize && tell(f) > fsize * blocksize)
					break;
			}
			else
				printf("check error at %d in %s\n", tell(f), n);
		fsize = tell(f) / blocksize;
	}
	if (!fsize)
	/* this does not work - i don't know why: */
		fsize = lseek(f, 0, SEEK_END);
	while (1) /* for now we stop by pressing ^C. */
	{
		if (lseek(f, blocksize * (rand() % fsize), SEEK_SET) < 0)
			printf("seek error %d\n", errno);
		else
